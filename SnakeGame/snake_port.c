...
#define SNAKE_TCP_SERVER_PORT	(uint16_t)(8000u)
#define SNAKE_UDP_SERVER_PORT	(uint16_t)(8001u)
...
static void platform_control_init(void)
{
	  ...
	  tcp_server_init(SNAKE_TCP_SERVER_PORT);

	  /* Start TCP server on the address 192.168.100.1:8001
	   * and sending response to client's port 80001 */
	  udp_server_init(SNAKE_UDP_SERVER_PORT, SNAKE_UDP_CLIENT_PORT);

	  /* Start listen on UART3 in ISR mode for control */
	  uart_rx_start();
}
